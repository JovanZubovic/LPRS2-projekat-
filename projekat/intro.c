...
int main(void) {
	...
	while(1){
		...
		if(previous == 0 && joypad.a == 0){
			toggle_active = 0;
			previous = 0;
		} 
		else if(previous == 0 && joypad.a == 1){
			toggle_active = 1;
			previous = 1;
		} 
		else if(previous == 1 && joypad.a == 1){
			toggle_active = 0;
			previous = 1;
		} 
		else if(previous == 1 && joypad.a == 0){
			toggle_active = 0;
			previous = 0;
		}
		...
		case RECT:
			...
			if(joypad.right && gs.rect.x == (SCREEN_W - RECT_W)){	
				mov_x = 0;
				gs.rect.x += mov_x*STEP;	
			}
			else if(joypad.left && gs.rect.x == 0){				
				mov_x = 0;
				gs.rect.x += mov_x*STEP;	
			}
			
			if(joypad.up && gs.rect.y == 0){
				mov_y = 0;
				gs.rect.y += mov_y*STEP;		
			}
			else if(joypad.down && gs.rect.y == (SCREEN_H - RECT_H)){
				mov_y = 0;
				gs.rect.y += mov_y*STEP;	
			}
			else{
			...
			break;
		case SQ:
			if(joypad.right && gs.sq.x == (SCREEN_W - SQ_A)){	
				mov_x = 0;
				gs.sq.x += mov_x*STEP;
			}
			else if(joypad.left && gs.sq.x == 0){
				mov_x = 0;
				gs.sq.x += mov_x*STEP;
			}
			
			if(joypad.up && gs.sq.y == 0){
				mov_y = 0;
				gs.sq.y += mov_y*STEP;
			}
			else if(joypad.down && gs.sq.y == (SCREEN_H - SQ_A)){
				mov_y = 0;
				gs.sq.y += mov_y*STEP;			
			}
			else{
		...
		for(int r = 0; r < SCREEN_H; r++){
			for(int c = 0; c < SCREEN_W; c++){
				pack_idx1_p32[r*(SCREEN_W) + c] = 0x00000000;
			}
		}

		for(int r = gs.rect.y; r < gs.rect.y+RECT_H; r++){
			for(int c = gs.rect.x; c < gs.rect.x+RECT_W; c++){
				pack_idx1_p32[r*(SCREEN_W/32) + c/32] = 0xffffffff;
			}
		}
